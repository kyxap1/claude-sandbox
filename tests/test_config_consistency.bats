...
@test ".env: every variable is referenced in devcontainer.json build args" {
    while IFS='=' read -r key _; do
        [[ -z "$key" || "$key" =~ ^# ]] && continue
        run jq -e --arg k "$key" '.build.args | has($k)' "$DEVCONTAINER_JSON"
        [ "$status" -eq 0 ] || fail ".env has $key but devcontainer.json build.args does not"
    done < "$DOTENV"
}
